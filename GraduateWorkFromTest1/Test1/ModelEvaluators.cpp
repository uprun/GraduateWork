...
void evaluateModelWith6ParamsMinimizeLineDistance( const double *par, int m_dat,
        ...
        const void *data, double *fvec, int *userbreak )
    {
		...
		for(int i = 0; i < upI; ++i)
		{
			...
			
			{
				...
				if(z <= 0.1)
				{
					fvec[2*i] = (z - 0.1) * (z - 0.1) * penalty;
					fvec[2*i+1] = (z - 0.1) * (z - 0.1) * penalty;
				}
				else
				{
					Array* currentPoint = new Array();
	...
	void evaluateModelPar7_WithProjectionDifferencesçPointsInSpace( const double *par, int m_dat,
        const void *data, double *fvec, int *userbreak )
    {
        /* for readability, explicit type conversion */
        data_struct *D;
